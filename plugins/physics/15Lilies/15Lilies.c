...
static int physics_enabled = 1; /* disable this plugin if geoms or bodies are not found, necessary to avoid Webots crashing */
static int i,j;
...
#define DRAG_COEF 1
//#define RAND_FORCE_AMPLITUDE 0*0.0001 // 5.5mN for lighter Lily modules, Lily robots are heavier
//#define RAND_FORCEx (SCALE*SCALE*SCALE*RAND_FORCE_AMPLITUDE) 
#define VEL_FIELD_FILE_NAME "flow_vel_23.txt"//"vel_field_total_100_-50.txt" //"flow_vel.txt" //
#define VEL_STD_FIELD_FILE_NAME "flow_vel_std_23.txt"
//#define Parameters "prob.txt"

static double VX[DIVISIONS][DIVISIONS];
static double VZ[DIVISIONS][DIVISIONS];

static double VX_STD[DIVISIONS][DIVISIONS];
static double VZ_STD[DIVISIONS][DIVISIONS];

double K_FLOW;
double K_flow_rand;
double RAND_FORCEx;
double RAND_FORCEy;
static const char Parameters[256]; 
...
static void computeDrag(const dReal *position_vect,const dReal* lin_velocity,dBodyID robot);
static void computeDrag_randn(const dReal *position_vect,const dReal* lin_velocity,dBodyID robot);
...
void webots_physics_step() {
	...
	if(dWebotsGetTime()/1000.0<2.0){
    	for(i=0; i<N_ROBOTS; i++){
    		...
    		for(j=0; j<N_ROBOTS; j++){
    			if(j!=i){
                      		//dWebotsConsolePrintf("%f %f\n",sqrt(SQ(position_vect2[0]-position_vect[0]) + SQ(position_vect2[2]-position_vect[2])),sqrt(SQ(-0.005)+SQ(0.0175)));
    				position_vect2 = dBodyGetPosition(lily_robots[j]);
    				if( sqrt(SQ(position_vect2[0]-position_vect[0]) + SQ(position_vect2[2]-position_vect[2]))<0.05 ){
    					dBodySetLinearVel(lily_robots[i], (position_vect2[0]-position_vect[0]), 0, (position_vect2[2]-position_vect[2]) );
	...
	for (i = 0; i < N_ROBOTS; i++) {
		...
		v_rel[X]=(lin_velocity[X]-flow[X])/SCALE; // relative velocity of flow to robot
		...
		v_rel[Z]=(lin_velocity[Z]-flow[Z])/SCALE;
                       ...
                       //dWebotsConsolePrintf("rel_velocity: %f, %f, %f \n", v_rel[X], v_rel[Y], v_rel[Z]);
...
static void getFlowVelocity(const dReal *position,dReal *flow_vel) {
	...
	x_index = (int) ((double)DIVISIONS * (position[X]/SCALE + ARENA_R) / (2.0*ARENA_R));
	z_index = (int) ((double)DIVISIONS * (position[Z]/SCALE + ARENA_R) / (2.0*ARENA_R));
	//dWebotsConsolePrintf("Indexes:[%d][%d]\n",x_index,z_index);       
	if(x_index<0 || x_index>(DIVISIONS-1) || z_index<0 || z_index>(DIVISIONS-1)){   
		flow_vel[X] = 0.0;
		flow_vel[Z] = 0.0;
	}
	else {
		flow_vel[X] = K_FLOW*VX[x_index][z_index];
		flow_vel[Z] = K_FLOW*VZ[x_index][z_index];
	}
	flow_vel[Y] = 0.0;
...
static void getFlowVelocity_randn(const dReal *position,dReal *flow_vel) {
	...
	if(x_index<0 || x_index>(DIVISIONS-1) || z_index<0 || z_index>(DIVISIONS-1)){   
		...
		flow_vel[X] = K_FLOW*randn(VX[x_index][z_index],VX_STD[x_index][z_index]);
		flow_vel[Z] = K_FLOW*randn(VZ[x_index][z_index],VZ_STD[x_index][z_index]);
	}
	flow_vel[Y] = 0.0;
	//dWebotsConsolePrintf("Flow speed: %f %f %f\n",flow_vel[X],flow_vel[Y],flow_vel[Z]); 
}
...
static void computeDrag(const dReal *position_vect,const dReal* lin_velocity, dBodyID robot){		
	...
	f_drag[Y] = -SIGN(lin_velocity[Y]) * 0.5 * WATER_DENSITY * LILY_SIZE_X * LILY_SIZE_Z * SQ(lin_velocity[Y]/SCALE);
	dBodyAddForce(robot,0,f_drag[Y]*SCALE*SCALE,0);
	
	// drag on side faces
	for(side=0;side<4;side++){
		for(i=0;i<DRAG_POINTS;i++) {
			switch(side) {
				case 0:		//front side (positive X)
					p[X]= LILY_SIZE_X/2.0;
					p[Y]= LILY_SIZE_Y/2.0;
					p[Z]= LILY_SIZE_Z/2.0 - LILY_SIZE_Z/2.0/(double)DRAG_POINTS -(double)i*LILY_SIZE_Z/(double)DRAG_POINTS;
					surf_normal[X] = -1.0;
					surf_normal[Y] = 0.0;
					surf_normal[Z] = 0.0;
					face_area = LILY_SIZE_X*LILY_SIZE_Y/DRAG_POINTS;
					break;
				case 1:		//right side (positive Z)
					p[X]= LILY_SIZE_X/2.0 - LILY_SIZE_X/2.0/(double)DRAG_POINTS -(double)i*LILY_SIZE_X/(double)DRAG_POINTS;
					p[Y]= LILY_SIZE_Y/2.0;
					p[Z]= LILY_SIZE_Z/2.0;
					surf_normal[X] = 0.0;
					surf_normal[Y] = 0.0;
					surf_normal[Z] = -1.0;
					face_area = LILY_SIZE_Z*LILY_SIZE_Y/DRAG_POINTS;
					break;
				case 2:		//back side (negative X)
					p[X]= -LILY_SIZE_X/2.0;
					p[Y]= LILY_SIZE_Y/2.0;
					p[Z]= LILY_SIZE_Z/2.0 - LILY_SIZE_Z/2.0/(double)DRAG_POINTS -(double)i*LILY_SIZE_Z/(double)DRAG_POINTS;
					surf_normal[X] = 1.0;
			...
			dBodyGetRelPointPos(robot,p[X]*SCALE,p[Y]*SCALE,p[Z]*SCALE,p_world); // get point in world coordinates
			dBodyGetPointVel(robot,p_world[X],p_world[Y],p_world[Z],v_point); // get point velocity
			//dWebotsConsolePrintf("getFlowVelocity called in next line");
			getFlowVelocity(p_world,flow); // get flow velocity at the point
			v_rel[X]=flow[X]-v_point[X]/SCALE; // relative velocity of flow to robot
			v_rel[Y]=flow[Y]-v_point[Y]/SCALE;
			v_rel[Z]=flow[Z]-v_point[Z]/SCALE;
			v_abs_sq = SQ(v_rel[X])+SQ(v_rel[Y])+SQ(v_rel[Z]); // relative velocity absolute value squared
			if(sqrt(v_abs_sq)<MIN_FLOW) // if flow is zero no drag calc is required
				return;
			dBodyVectorToWorld(robot,surf_normal[X],surf_normal[Y],surf_normal[Z],surf_normal); // rotate surface_normal to world coord system
			cos_angle = calcCosAngle(surf_normal,v_rel); // angle between flow and surface
			drag_abs = 0.5*DRAG_COEF*WATER_DENSITY*v_abs_sq*face_area*cos_angle;	// absolute value of the drag force (except for the cosine which reflects the orientation)
			//int forceScale = SCALE*100;
			int forceScale = SCALE*1;
			f_drag[X]= K_flow_rand*forceScale* drag_abs*v_rel[X]/sqrt(v_abs_sq); // drag force acts in a direction opposite to oncoming flow velocity
			f_drag[Y]= K_flow_rand*forceScale* drag_abs*v_rel[Y]/sqrt(v_abs_sq);
			f_drag[Z]= K_flow_rand*forceScale* drag_abs*v_rel[Z]/sqrt(v_abs_sq);
			//f_drag[Z]= SCALE*SCALE*SCALE*SCALE* drag_abs*v_rel[Z]/sqrt(v_abs_sq);
			dBodyAddForceAtPos(robot,f_drag[X],f_drag[Y],f_drag[Z],p_world[X],p_world[Y],p_world[Z]);
			//dWebotsConsolePrintf("Drag: %f %f %f",f_drag[X],f_drag[Y],f_drag[Z]);
			//dWebotsConsolePrintf("Flow speed: %f %f %f\n",flow[X],flow[Y],flow[Z]); 
			//dWebotsConsolePrintf("lin_vel: %f %f %f",lin_velocity[X],lin_velocity[Y],lin_velocity[Z]);
			//dWebotsConsolePrintf("Cos_angle: %f",cos_angle);
			//dWebotsConsolePrintf("Normal: %f %f %f",surf_normal[X],surf_normal[Y],surf_normal[Z]);
			//dWebotsConsolePrintf("Vrel: %f %f %f",v_rel[X],v_rel[Y],v_rel[Z]);
			//dWebotsConsolePrintf("p_world: %f %f %f",p_world[X],p_world[Y],p_world[Z]);
			//dWebotsConsolePrintf("p_rel: %f %f %f",p[X],p[Y],p[Z]);
		}
	}
	//dWebotsConsolePrintf("lin_vel: %f %f %f",lin_velocity[X],lin_velocity[Y],lin_velocity[Z]);
}
...
static dReal calcCosAngle(dVector3 surf_norm,dVector3 field){
	dReal dot,abs;
	dot = surf_norm[X]*field[X]+surf_norm[Y]*field[Y]+surf_norm[Z]*field[Z];
	if(dot<0.0)
		dot = 0.0; // only take into account flow going into the faces of the cube, not going out from them
...
static void addRandomForce(dBodyID robot){
	double rnd_f_x,rnd_f_z;
	rnd_f_x = randn(0,RAND_FORCEx);
	rnd_f_z = randn(0,RAND_FORCEy);
	dBodyAddForce(robot, rnd_f_x, 0.0, rnd_f_z);
...
static void addRandomForce_radial(dBodyID robot,const dReal *p){
	...
	dBodyAddForce(robot, -rnd_f*p[X]/sqrt(SQ(p[X])+SQ(p[Y])+SQ(p[Z])), 0.0, -rnd_f*p[Z]/sqrt(SQ(p[X])+SQ(p[Y])+SQ(p[Z])));
