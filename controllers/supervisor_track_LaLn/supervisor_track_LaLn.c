...
#define ARENA_R                                                                \
  0.2 // maximum radius of initial position of robots is arena_radius - 3 cm
#define SQ(x) ((x) * (x))

#define MAX_TIME 4 * 3600 // 2*3600 // seconds
#define LOG_START_TIME                                                         \
  0.0 // Start logging after some time, to discard transient state
...
static int macro_state_labels_unique[N_MACRO_CONFIGS] = {
    0, 2, 4, 6,
    8}; //{ 0,2,4,6,8,10 }; // unique labels that indicate configuration types
static int macro_state_labels_unique_size[N_MACRO_CONFIGS] = {
...
// static int
// prob_test[nb_prob]={RAND_MAX/1000,RAND_MAX/625,RAND_MAX/125,RAND_MAX/25,RAND_MAX/10,RAND_MAX/5,(RAND_MAX/5)*2,RAND_MAX/2,(RAND_MAX/5)*4};
/////////////////////////////
...
int main() {
  ...
  for (i = 0; i < N_ROBOTS; ++i) {
    ...
    robot_field_pos[i] =
        wb_supervisor_node_get_field(robot_node[i], "translation");
    robot_field_orient[i] =
        wb_supervisor_node_get_field(robot_node[i], "rotation");
  }
...
static bool supervisor_init(void) {
  ...
  for (j = 0; j <= N_ROBOTS; j++) {
    ...
    robot_field_pos[ii] =
        wb_supervisor_node_get_field(robot_node[ii], "translation");
    robot_field_orient[ii] =
        wb_supervisor_node_get_field(robot_node[ii], "rotation");
    robot_field_data[ii] =
        wb_supervisor_node_get_field(robot_node[ii], "la_ln");
  ...
  wb_supervisor_field_set_sf_float(gravity_field, grav);
...
static int supervisor_step(void) {
  ...
  if (sim_time > print_counter * 720) {
    printf("time %f \n", sim_time);
    printf("macro state %d %d %d %d %d \n", macro_state[0], macro_state[1],
           macro_state[2], macro_state[3], macro_state[4]);
    // printf("labels %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d \n",
    // labels[0], labels[1], labels[2], labels[3], labels[4], labels[5],
    // labels[6], labels[7], labels[8], labels[9], labels[10], labels[11],
    // labels[12], labels[13], labels[14]);
    printf("deltaT %f %f %f \n", deltaT[0], deltaT[1], deltaT[2]);
    print_counter = print_counter + 1;
  }
  if (sim_time > time_end) {
    return_flag = 1;
    printf("Time ended\n");
  }
  return return_flag;
...
static void position_init(void) {
  ...
  for (i = 0; i < N_ROBOTS; i++) {
    ...
    do {
      ...
      if (SQ(pos_x[i]) + SQ(pos_y[i]) <
          SQ(ARENA_R)) { // if inside arena check collision between robots
        collision = 0;
        for (j = 0; j < i; j++) {
          if (SQ(pos_x[i] - pos_x[j]) + SQ(pos_y[i] - pos_y[j]) <
              SQ(ROBOT_DIST)) { // if robots are too close generate a new value
            collision = 1;
          }
        }
      }
...
void evaluate() {
  ...
  for (n = 0; n < N_ROBOTS; ++n) {
    ...
    for (i = 0; i < PROB_SIZE; i++)
      index +=
          sprintf(&label_field[index], "%d ", (int)(RAND_MAX * prob_this[i]));
  ...
  do {
    ...
    if (macro_state[N_MACRO_CONFIGS - 1] -
            macro_state_prev[N_MACRO_CONFIGS - 1] !=
        0) {
  ...
  } while (wb_robot_step(time_step) != -1 && t < MAX_TIME &&
           init_success == true);
